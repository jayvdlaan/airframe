...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Schema registry with a simple migration from v1 -> v2
    ...
    // Build protected typed repo using JSON
    let builder = SDataProtectedBuilder::new()
        .bytes(bytes)
        .context(ctx)
        .registry(reg.clone());
    let repo: ProtectedTypedRepo<_, _, _, UserV2> = builder.build_typed(JsonCodec)?;
    ...
    repo.put(&k, &v2)?;
    let out = repo.get(&k)?.unwrap();
    assert_eq!(out, v2);

    println!("protected_typed_mem ok: {:?}", out);
    Ok(())
}
