...
#[test]
fn protected_typed_repo_roundtrip_with_migration() {
    ...
    let builder = SDataProtectedBuilder::new()
        ...
        .context(ctx)
        .registry(reg.clone());
    let repo: ProtectedTypedRepo<_, _, _, UserV2> = builder.build_typed(JsonCodec).unwrap();

    let k = Key::new("alice").unwrap();

    // Minimal roundtrip using protected repo (encryption handled by pdata under the hood)
    let v2 = UserV2 {
        id: "alice".into(),
        name: "Alice".into(),
        age: 0,
    };
    repo.put(&k, &v2).unwrap();
